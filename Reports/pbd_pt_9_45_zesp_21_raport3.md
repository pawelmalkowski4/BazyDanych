...
```
Tabela zawierająca dni wolne od pracy.
| Nazwa atrybutu | Typ | Opis/Uwagi |
| -------------- | --- | ---------- |
| CalendarDate   |Date | Klucz główny będący datą
|DayOfWeekName| NVARCHAR(20)| Dzień tygodnia|
|IsWorkDay|BIT| Flaga wskazująca, czy dzień jest dniem wolnym od pracy| 
...
|Status|VARCHAR(20)|Określa fazę, w której aktualnie znajduje się zamówienie. `'Pending'` (oczekujące), `'In Progress'` (realizowane), `'Completed'` (zakończone), `'Cancelled'` (anulowane). Wartość domyślna to `'Pending'`.|
...
```
Jest to tabela łącząca zamówienia (`Orders`) i ich opisy (`OrderDetails`) wraz z planowaną produkcją (`ProductionPlan`) umożliwia to rezerwacje produktów, które dopiero zostaną wyprodukowane.
| Nazwa atrybutu | Typ | Opis/Uwagi 
| -------------- | --- | ---------- |
|OrderID|INT|Identyfikator zamówienia. Jest to klucz obcy wskazujący na tabele zamówień (Orders). Składa się on na klucz główny zamówień produkcyjnych. Wraz z ProductID tworzy klucz obcy wskazujący na szczegóły zamówień|
|ProductID|INT|Współtworzy wraz z OrderID klucz obcy wskazujący na tabele szczegółów zamówienia|
...
### 2. Aktualizacja dostępnej liczby produktów po złożeniu zamówienia
